...
`!정보 <ニックネーム>`, `!무릉 <ニックネーム>`, `!유니온 <ニックネーム>`, `!유챔 <ニックネーム>`, `!장비 <ニックネーム>`, `!경험치 <ニックネーム>`, `!심볼 <地域> <開始> <目標>`, `!심볼만렙`, `!보스`, `!계산기 <ボス> <難易度> [人数]`, `!보스수익`, `!보스보상`, `!보스렙뻥`, `!보스포뻥`, `!메카베리 <レベル>`, `!메포효율`, `!공지`, `!이벤트`, `!썬데이`, `!선데이`, `!인벤`, `!마빡도로시`, `!디코`を提供します。

`!계산기 검마 하드 2인 / 세렌 노말 3인`は、`価格 ÷ パーティー人数`の小数点以下を切り捨てて1人分の結晶収益を計算します。`!보스수익`も同じエイリアスです。どちらもバージョン管理された静的データを使用し、実行時に参照ページへアクセスしません。
