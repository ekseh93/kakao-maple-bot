...
## 2026-09-08 후속 변경 CI 복구와 포트폴리오 검수

- 증상: 후속 작업 브랜치의 최신 원격 검사가 포맷 단계에서 실패해 테스트·빌드가 실행되지 않았습니다.
- 재현: 통합 전 스냅샷의 `pnpm format:check`는 Lambda 파일을 지목했고, 독립 실행한 Vitest는 191개가 통과했습니다. 테스트 실패로 단정하지 않았습니다.
- 수정: 해당 파일 포맷을 적용하고 CI를 quality, security, test-build로 나눴습니다. 최종 `verify`는 세 검사가 모두 성공해야 통과합니다.
- 추가 발견: 선택 정보인 대기질 API의 JSON 파싱과 필드 검증이 실패 격리 밖에 있었고, 본문을 읽기 전에 제한 타이머가 해제됐습니다. 본문 처리까지 제한과 오류 처리를 유지하도록 수정했습니다.
- 공개 검수: 개인용 발신자·응답의 하드코딩을 비공개 환경설정으로 옮겼습니다. 새 공개 스냅샷에서 실제 값은 제거하고 테스트는 가상 값으로 교체했습니다. 기존 브랜치 이력 제거를 수행한 것은 아닙니다.
- 검증: Vitest 196개(가상 타이머를 사용한 본문 지연, JSON·필드 오류, 미설정 명령 차단 포함). 전체 검사와 원격 CI 결과는 연결된 PR에서 추적합니다.
- 재발 방지: PR 필수 규칙, 독립 CI 결과, 일본어 사례와 대상 버전별 검증 기록을 연결합니다.
- AWS/Android: 이번 작업에서 재배포나 단말 변경은 수행하지 않습니다.
- 추적: [Issue #10](https://github.com/ekseh93/kakao-maple-bot/issues/10).
...
## 2026-09-07 전 세계 날씨 주 응답 우선 처리

- 후보를 비교한 결과 Open-Meteo는 전 세계 지오코딩·예보를 API 키 없이 제공해 현재 프로젝트의 무료·비상업 운영 조건에 가장 적합했습니다. WeatherAPI는 키가 필요하고, MET Norway는 식별 가능한 `User-Agent`와 별도 이용 제한을 요구하므로 교체하지 않았습니다.
- 날씨와 대기질 요청은 병렬로 유지하되, 대기질 요청에는 1.2초 소프트 타임아웃을 적용했습니다. 대기질 API가 느리거나 5xx/네트워크 오류를 내도 `!날씨`의 기온·습도 응답은 전송하며 PM 값은 `정보 없음`으로 표시합니다.
- Open-Meteo 지오코딩 좌표(1시간)와 최종 날씨 결과(5분) 캐시는 유지해 반복 조회의 전송량과 지연을 줄입니다.
