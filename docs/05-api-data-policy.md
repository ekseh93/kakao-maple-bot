...
## 6-1. 외부 상품·영화관 조회 facade

- 상품·매장·재고·영화관 조회는 `https://mcp.aka.page`의 공개 GET facade를 사용합니다.
- LLM이나 MCP 프로세스를 실행하지 않고 Lambda가 필요한 action만 직접 호출합니다.
- `submitDeveloperRequest` 같은 외부 등록 기능과 구매·예약·좌석 변경 기능은 연결하지 않습니다.
- 가격·재고·상영 정보는 조회 시점의 참고값이며 실제 매장·극장 상태와 다를 수 있습니다.
- `!주유소`는 동일 호스트의 오피넷 읽기 전용 엔드포인트(`/api/opinet/lowest`)에 휘발유와 상위 3개 조건을 전달하며, 주유소 구매·예약·회원 기능은 호출하지 않습니다.
- `!환율`은 API 키 없는 공개 환율 endpoint에서 USD 기준 KRW·JPY를 읽고 USD/KRW와 JPY/KRW를 계산합니다. 거래·환전 주문은 수행하지 않습니다.
- `!주변`은 공개 장소 검색 endpoint에서 지역·선택 카테고리로 최대 5개 장소를 읽습니다. `!상품상세`는 상품 ID 상세 조회만 수행하며 구매·주문·결제 기능은 연결하지 않습니다.
