...
| KRX Open API   | 국내 종목·일별 가격 정보 조회      | 주문, 계좌 접근, 자동매매               | 선택      |
| Tiingo         | 미국 종목 검색·일별 가격 정보 조회 | 주문, 계좌 접근, 자동매매               | 선택      |
| TMDB           | Netflix 제공작 메타데이터 조회     | 스트리밍 제공, 콘텐츠 복제·재배포       | 선택      |
...
MVP는 국내·미국 주식의 읽기 전용 일별 시세만 허용합니다.

- 국내: KRX Open API 종목기본정보와 일별매매정보
- 미국: Tiingo 검색과 일별 가격
- 주문, 계좌번호, 잔고, 보유 종목, 자동매매, 매매 신호는 호출하지 않습니다.
- 인증정보는 Lambda 환경변수로만 주입하고 공기계에 저장하지 않습니다.

공식 문서: [KRX Open API](https://openapi.krx.co.kr/contents/OPP/INFO/service/OPPINFO004.cmd), [Tiingo EOD](https://www.tiingo.com/documentation/end-of-day)
...
## 6-1. 다이소 상품 조회 facade

- 다이소 상품 조회는 `https://mcp.aka.page`의 공개 GET facade를 사용합니다.
- LLM이나 MCP 프로세스를 실행하지 않고 Lambda가 필요한 action만 직접 호출합니다.
- `submitDeveloperRequest` 같은 외부 등록 기능과 구매·예약·좌석 변경 기능은 연결하지 않습니다.
- 상품·가격·픽업 정보는 조회 시점의 참고값이며 실제 상태와 다를 수 있습니다.
- 퀘이사존·디시인사이드 공개 게시판은 고정된 식별 헤더와 제한된 읽기 재시도만 사용하며, User-Agent 로테이션·프록시·IP 변경·검사 우회·접근제어 위조는 하지 않습니다. 정상 결과는 5분간 캐시하고, 일시적인 접근 제한·응답 오류가 발생하면 최근 6시간 이내의 마지막 정상 결과를 임시 표시합니다. 캐시가 없거나 6시간이 지나면 사용자용 오류를 반환합니다.
- `!주유소`는 동일 호스트의 오피넷 읽기 전용 엔드포인트(`/api/opinet/lowest`)에 휘발유와 선택한 시·도 코드, 상위 3개 조건을 전달하며, 주유소 구매·예약·회원 기능은 호출하지 않습니다.
- `!환율`은 API 키 없는 공개 환율 endpoint에서 USD 기준 KRW·JPY를 읽고 USD/KRW와 JPY/KRW를 계산합니다. 거래·환전 주문은 수행하지 않습니다.
- 서비스 정책·호출 제한·응답 구조 변경을 정기적으로 확인하고, 장애 시 사용자용 오류만 반환합니다.

## 6-2. 보스 결정 가격과 수익 계산

- `!보스`와 `!보스수익`은 저장소가 독립적으로 관리하는 정적 결정 가격을 사용합니다.
- 참고 계산기(<https://matsu1207.tistory.com/1226>)의 HTML·JavaScript·이미지·문구를 복제하거나 런타임에 호출하지 않습니다.
- 가격·난이도·주기·최대 파티 인원처럼 계산에 필요한 사실만 수동 검토해 프로젝트 소유 데이터 구조로 관리합니다.
- 가격표(<https://matsu1207.tistory.com/757>)와 계산기 동작은 교차 확인 자료이며, 패치 적용 전 공식 공지·가이드와 게임 내 표본을 우선 확인합니다.
- 가격, 주기 또는 인원 제한이 바뀌면 데이터 기준일과 golden fixture를 같은 변경에서 갱신합니다.
