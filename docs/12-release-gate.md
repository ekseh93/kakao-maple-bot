...
이 문서는 로컬 자동 검증 이후에 사용하는 수동 절차입니다. AWS 배포는 사용자 승인을 받은 뒤 수행하며, 관측한 결과만 기록합니다.
...
- 도쿄 리전 `ap-northeast-1`에 Terraform으로 9개 리소스를 생성했습니다.
- Lambda `kakao-maple-bot`과 API Gateway HTTP API가 생성되었습니다.
- 공개 `/health` URL을 호출해 `200`과 `{"status":"ok"}`를 확인했습니다.
- `BOT_ENABLED=false`이며 secret은 비어 있어 실제 카카오 릴레이 기능은 활성화하지 않았습니다.

## 금지 범위

Maple.GG와 Maplescouter는 계속 링크 전용입니다. 주문·계좌 API, 광고·초대·불특정 다수 선제 알림, 대화 원문 저장은 출시 게이트에서도 허용하지 않습니다. 단, ADR-006의 명시적 동의 방 주간 리마인더는 별도 제한 범위로 허용합니다.
