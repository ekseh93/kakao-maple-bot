...
2026-09-08 구현 확인: 아래 표의 Cache API/KV는 초기 설계 후보입니다. 현재 Lambda의 캐시, 중복 이벤트 ID, rate-limit 버킷은 `apps/lambda/src/index.ts`의 모듈 변수와 Map에 있으며 실행 환경 내부에서만 유지됩니다. 콜드 스타트·동시 인스턴스 간 공유, 분산 중복 제거를 보장하지 않습니다. DynamoDB는 익명 누적 카운터 용도로 사용하며 이 메모리 상태를 대신하지 않습니다.
...
`!통계`용으로는 도쿄 리전 DynamoDB의 `TOTAL` 단일 항목에 익명 누적 숫자와 갱신 시각만 원자적으로 저장합니다. 방·발신자·메시지 원문은 이 항목에도 저장하지 않습니다.
