...
- 대화 저장, 자동 친구 추가, 불특정 다수 대상 선제 메시지
...
실제 방 이름과 발신자명을 전송해야 하는 MessengerBot 제약이 있으면 서버 진입 즉시 정규화·해시하고 로그에는 남기지 않습니다. 사전 동의된 방의 주간 리마인더처럼 명시적으로 설정된 예약 발신만 공기계 릴레이가 담당합니다.
