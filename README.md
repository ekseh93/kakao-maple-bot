...
| 악몽선경 경험치 계산        | !악몽 닉네임                            | 최신 경험치율과 악몽선경 1판 경험치로 레벨업 판수 예상   |
| 앵글러 컴퍼니 경험치 계산   | !앵글 닉네임                            | 최신 경험치율과 앵글러 1판 경험치로 레벨업 판수 예상     |
| 하이마운틴 경험치 계산      | !마운틴 닉네임                          | 최신 경험치율과 하이마운틴 1판 경험치로 레벨업 판수 예상 |
...
| 보스 결정 개인 수익 계산    | !보스수익 검마 하드 2인 / 세렌 노말 3인 | 정적 결정 가격과 파티 인원 기반 자체 계산                |
| 일반 계산·수수료 n빵         | !계산기 12퍼 x 11개 / 25.3억 2명 5퍼     | 사칙연산과 메소 수수료·균등 분배                          |
| 백옥·흑옥 보스 반지 상자     | !시드링                                 | 백옥 5회·흑옥 5회 확률 미니게임                           |
| 칠흑 장신구 상자            | !칠흑깡                                 | 가중치 조정 보상 목록 기반 1회 미니게임                   |
...
| VIP 사우나 경험치           | !사우나 레벨 또는 닉네임                | 레벨별 또는 캐릭터 현재 경험치 기준 레벨업 시간          |
| 메포 효율                   | !메포효율                               | 284레벨 기준 메포 대비 경험치 효율표                     |
| 외부 상세보기               | 캐릭터 조회 응답의 링크                 | Maple.GG 링크만 생성                                     |
| 랜덤 게임                   | !가위, !바위, !보, !치킨vs짬뽕          | 자체 로직                                                |
| 메뉴 추천                   | !뭐먹지                                 | 요리·사이드·디저트·술안주, 소재획·재획비 각 31배          |
...
`!보스수익`은 검은 마법사·그란디스 보스를 대상으로 `결정 가격 ÷ 파티원 수`의 소수점 이하를 버려 개인 수익을 계산합니다. `/`로 여러 보스를 구분하고, 인원을 생략하면 1인으로 계산합니다. 예: `!보스수익 검마 하드 2인 / 세렌 노말 3인`. 실행 중 외부 페이지를 호출하지 않습니다.

`!계산기`는 일반 사칙연산용 별도 명령입니다. `!계산기 12 x 11`은 `132`, `!계산기 12퍼 x 11개`는 `132퍼`를 반환합니다. `+`, `-`, `x`, `×`, `*`, `/`, `÷`, 괄호와 소수를 지원하며 코드 실행 함수는 사용하지 않습니다. `!계산기 2,530,000,000 2인 3%` 또는 `!계산기 25.3억 2명 5퍼`처럼 입력하면 수수료를 뺀 n빵도 계산합니다.
