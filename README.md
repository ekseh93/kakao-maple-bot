...
- 국내 주식은 KRX Open API, 미국 주식은 Tiingo, 일본 주식은 Yahoo Finance 공개 조회를 선택형 공급자로 사용하며 투자 권유 기능은 만들지 않습니다. Yahoo Finance 경로는 공식 인증 API가 아니므로 응답 형식 변경이나 일시 차단 가능성이 있습니다.
...
| 일본여행 추천               | !일본여행                             | 저장소 내 정적 여행지 목록                     |
| 오늘의 운세                 | !운세 00년생                          | 저장소 내 정적 오락용 문구                     |
...
| 주식                        | !주식 이름                            | KRX(한국) + Tiingo(미국) + Yahoo Finance(일본) |
...
## 구조
    ...
    ├─ Stock adapter ─ KRX / Tiingo / Yahoo Finance
