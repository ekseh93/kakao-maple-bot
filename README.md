...
| 보스 결정 개인 수익 계산    | !계산기 검마 하드 2인 / 세렌 노말 3인   | 정적 결정 가격과 파티 인원 기반 자체 계산                |
...
`!계산기`와 `!보스수익`은 검은 마법사·그란디스 보스를 대상으로 `결정 가격 ÷ 파티원 수`의 소수점 이하를 버려 개인 수익을 계산하는 같은 명령입니다. `/`로 여러 보스를 구분하고, 인원을 생략하면 1인으로 계산합니다. 예: `!계산기 검마 하드 2인 / 세렌 노말 3인`. 실행 중 외부 페이지를 호출하지 않습니다.
