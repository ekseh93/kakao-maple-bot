...
공기계의 카카오톡 계정을 입구로 사용해 메이플스토리 정보, 심볼 계산, 랜덤 게임, 메뉴 추천, 국내·미국 주식 시세를 제공하는 개인용 챗봇 프로젝트입니다.
...
- 국내 주식은 KRX Open API, 미국 주식은 Tiingo를 선택형 공급자로 사용하며 투자 권유 기능은 만들지 않습니다.
...
| 전 세계 현재 날씨           | `!날씨 지역명`                        | Open-Meteo 공식 API                         |
...
| 메뉴 추천                   | !뭐먹지                               | 저장소 내 정적 데이터                       |
| 주식                        | !주식 삼성전자 / !주식 Tesla          | KRX Open API(국내) + Tiingo(미국)           |
...
구현된 범위는 Phase 0 기반, Phase 1 순수 명령, Phase 2 인증·방 권한·kill switch·중복 이벤트 TTL·방 rate limit, Phase 3 Nexon 어댑터와 링크 전용 외부 상세보기, Phase 4 MessengerBot R 릴레이 스크립트와 운영 문서, Phase 5 읽기 전용 KRX/Tiingo 주식 어댑터, Phase 6 로컬 안전성 보완입니다. 공급자 테스트는 fetch mock으로 수행하며, 승인된 도쿄 AWS Lambda 배포와 API smoke test만 별도로 수행했습니다.
